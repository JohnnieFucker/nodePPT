...
var dataModel = require('../models/data_tables.js');
    ...
    var app = connect(
        ...
        function(req, res) {
            ...
            if(dirname=='/getPPT'){
                ...
                var dataId = path.basename(url);
                try {
                    dataId = decodeURIComponent(dataId);
                } catch (e) {
                    dataId = path.basename(url);
                }
                getPPTMD(dataId,res);
                return;
            }
...
function getPPTMD(item_id,res){
    dataModel.findById(item_id,function(err,dataObj){
        if(err){
           res.send(err);
        }
        var html = $.renderFile(templateDir + 'pptTemplate.ejs', dataObj);
        fs.writeFile(path.join(rootDir, 'ppts')+'/'+item_id+'.md', html,function(err1,result){
            if(err1){
                console.log(err1);
                return;
            }
